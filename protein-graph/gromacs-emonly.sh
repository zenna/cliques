...
# Copyright (c) 2009-10 Yun William Yu
...
#echo -e "y\ny\ny\ny\ny\ny\ny\ny\ny\ny\ny\ny" | g_pdb2gmx -f ${name}.pdb -o ${name}allH.gro -p ${name}allH.top -i -q -n -ff oplsaa >& 01output.pdb2gmx
echo -e "y\ny\ny\ny\ny\ny\ny\ny\ny\ny\ny\ny" | g_pdb2gmx -f ${name}.pdb -o ${name}allH.gro -p ${name}allH.top -i -q -n -ff oplsaa -merge -ignh >& 01output.pdb2gmx
# Apparently, newer versions of Gromacs don't have the merge option anymore and have replaced it with "chainsep", which I don't yet know how to use.
# Additional note: I went back to v4.0.7, which works with this line, as opposed to 4.5.1, which does not.
...
g_editconf -f ${name}allH.gro -o ${name}allHbox.gro -bt cubic -d 0.6 >& 02output.editconf_box
...
g_genbox -cp ${name}allHbox.gro -p ${name}allH.top -o ${name}allHsol.gro -cs >& 03output.genbox
...
g_grompp -f dummy.mdp -po dummyout.mdp -c ${name}allHsol.gro -p ${name}allHsol.top -pp ${name}allHsol_pre.top -o ${name}allHsol.tpr >& 07output.grompp_genion
...
echo SOL | g_genion -s ${name}allHsol.tpr -o ${name}allHion.gro -p ${name}allHsol_pre.top -pname NA+ -g genion.log -pot ${name}allHion.pdb -nname CL- -conc 0.0001  -neutral >& 08output.genion
...
g_grompp -f ${name}preEMsteep.mdp -po ${name}_EMsteep.mdp -c ${name}allHion.gro -p ${name}allHion.top -pp ${name}allHion_out.top -o ${name}allHion.tpr >& 11output.grompp_emsd2
if [ "$?" -ne "0" ] ; then
...
g_mdrun -s ${name}allHion.tpr -o ${name}_EMsteep.trr -c ${name}_EMsteep.gro -e ${name}_EMsteep.edr -g ${name}_EMsteep.log -nice 2 -v >& 12output.mdrun_emsd
#if [ "$?" -ne "0" ] ; then
...
g_grompp -f ${name}preEMcg.mdp -po ${name}_EMcg.mdp -c ${name}_EMsteep.gro -p ${name}allHion.top -pp ${name}preEMsteep.top -o ${name}_EMcg.tpr >& 13output.grompp_emcg
if [ "$?" -ne "0" ] ; then
...
g_mdrun -s ${name}_EMcg.tpr -o ${name}_EMcg.trr -c ${name}_EMcg.gro -e ${name}_EMcg.edr -g ${name}_EMcg.log -nice 2 -v >& 14output.mdrun_emcg
#if [ "$?" -ne "0" ] ; then
...
echo 10 | g_energy -f ${name}_EMcg.edr -o energy.xvg >& 19output.g_energy
# generation of pdb
g_editconf -f ${name}_EMcg.gro -o ${name}_MD.pdb -bt cubic -d 0.6 >& 20output.editconf_pdb
