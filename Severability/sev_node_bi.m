...
%   NB: Searches both ways on directed links.
%
...
adj_nodes=union(find(sum(adj_matrix(start_node,:),1)) , ...
                find(sum(adj_matrix(start_node,:),2)) )  ;
...
while (length(community)<max_size)
    ...
    adj_nodes=union(find(sum(adj_matrix(community,:),1)) , ...
                    find(sum(adj_matrix(community,:),2)) )  ;
    %adj_nodes = find(sum(adj_matrix(community,:),1)); % Only want the adjacent nodes
    neighbours = setdiff(adj_nodes,community);
    % Only want nodes adjacent to complement
    complement_adj = union( find(sum(adj_matrix(setdiff(1:graph_size,community),:)),1) , ...
                            find(sum(adj_matrix(setdiff(1:graph_size,community),:)),2) ) ; 
...
%comm_final
[comm_final sev_1]=sev_steepest_descent_bi(adj_matrix, comm_final, t, max_size);
