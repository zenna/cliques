...
void Quick_sort(int low,int high,double arr[])
{
  ...
  //Declarations
		...
		for(int x2=0;x2<x1;x2++)
		{
           ...
           // for(int i=0;i<nCommunities[x2]+1;i++)
			...
			for(int i=0;i<nCommunities[x1]+1;i++)
			{
				...
				{
					if(matrix[i][j]>0)
						info-=2.0 * matrix[i][j] * log(matrix[i][j] * norma /(mh[i] * mv[j]));
				}
			}
    ...
    if(nlhs>2){
        ...
        if(((nTrials)*(nTrials-1))/2 % 2){
            ...
            median=(vi_saves[int(ceil(((nTrials)*(nTrials-1))/4.0))]+vi_saves[int(ceil(((nTrials)*(nTrials-1))/4.0))-1])/2;
        }
