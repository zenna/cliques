...
vector<int> n2c(levels[0].size());
  if (display_level==-1) {
   ...
   plhs[0] = mxCreateDoubleMatrix(levels[0].size(),2,mxREAL);
    double * output_tab = (double*) mxGetPr(plhs[0]);
    for (unsigned int node=0 ; node<levels[0].size() ; node++){
        output_tab[node] = (double) node;
        output_tab[levels[0].size()+node] = (double) n2c[node];
