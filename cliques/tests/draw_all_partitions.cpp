...
int main() {
    ...
    std::unordered_set<DjPartition,cliques::partition_hash, cliques::partition_equal> all_partitions;

    cliques::find_connected_partitions(orange_graph, all_partitions);
    cliques::draw_graph canvas(orange_graph);

    int index = 0;
    for (std::unordered_set<DjPartition,cliques::partition_hash, cliques::partition_equal>::iterator itr = all_partitions.begin();
