...
int main(int ac, char* av[]) {
	...
	std::vector<double> markov_time = { 0.1 };
	cliques::find_weighted_linearised_stability fnc(markov_time);
	std::vector<int> v1 = {0, 0, 1, 2, 2, 3, 3, 4 };
	std::vector<int> v2 = {0, 0, 1, 2, 2, 3, 3, 3 };
//	std::vector<int> v2 = {1, 1, 0, 2};
//	std::vector<int> v3 = {2, 1, 0, 0};
//	std::vector<int> v4 = {0, 1, 2, 2};

	cliques::VectorPartition p1(v1);
	cliques::VectorPartition p2(v2);
//	cliques::VectorPartition p3(v3);
//	cliques::VectorPartition p4(v4);


	cliques::LinearisedInternals i1(orange_graph, weights, p1);
	cliques::LinearisedInternals i2(orange_graph, weights, p2);
//	cliques::LinearisedInternals i3(orange_graph, weights, p3);
//	cliques::LinearisedInternals i4(orange_graph, weights, p4);


//	double stability = fnc(i1, 0);
	cliques::output("stability", fnc(i1, 0),fnc(i2, 0));

	cliques::output("Finding stabilities");
	std::ofstream stabs_file;
	stabs_file.open("stabs.mat");
	std::vector<double> markov_times = { 10.0, 0.3, 0.5, 0.8, 0.9, 0.95, 1.0,
			1.05, 1.1, 1.2, 2, 5, 10, 20,50, 100, 1000 };
	...
	cliques::sample_maxima(orange_graph, weights,func,diff_func,all_maxima,all_partitions,no_logging);
