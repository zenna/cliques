...
struct make_partition_colour_map {
	...
	std::map<int, cliques::xyz_colour<float> > operator () ()
	{
    	...
    	for (typename P::PartIterator pitr = partition_.begin(); pitr != partition_.end(); ++pitr) {
    		for (typename P::NodeIterator nitr = pitr.begin(); nitr != pitr.end(); ++nitr ) {
    			// insert new colour into map
    			colour_map.insert(std::pair<int,
    					cliques::xyz_colour<float> >(*nitr, cliques::xyz_colour<float>(hue,0.7,0.7)));
    		}
...
struct highlight_group_node_map {
	...
	highlight_group_node_map(P &partition, int highlighted_set)
		:partition_(partition), highlighted_set(partition_.find_set(highlighted_set))
	{}
    	...
    	for (typename P::PartIterator pitr = partition_.begin(); pitr != partition_.end(); ++pitr) {
    		for (typename P::NodeIterator nitr = pitr.begin(); nitr != pitr.end(); ++nitr ) {
    			...
    			if (partition_.find_set(*nitr) == highlighted_set) {
    				colour_map.insert(std::pair<int,
    					cliques::xyz_colour<float> >(*nitr, cliques::xyz_colour<float>(1.0,0.7,0.7)));
    			}
    			else {
    				colour_map.insert(std::pair<int,
    				    cliques::xyz_colour<float> >(*nitr, cliques::xyz_colour<float>(0.0,0.0,0.3)));
    			}
    		}
    	}
    	return colour_map;
...
/**
@brief This function looks at a graph and energies of each node and creates an edgemap
*/
template <typename G, typename M>
void create_flow_network(G &graph, std::vector<float> energies, M &map) {
	int alpha;
}
...
struct make_energy_edge_colour_map {
	...
	std::map<int, cliques::xyz_colour<float> > operator () ()
	{
		...
		if (DYNAMIC_SCALE == true) {
			for (std::vector<float>::iterator itr = energies_.begin();
					itr != energies_.end(); ++itr) {
				if (*itr > max_weight) {
					max_weight = *itr;
				}
				if (*itr < min_weight) {
		...
		for (std::vector<float>::iterator itr = energies_.begin();
			...
			hue = (*itr + shift)/range;
			colour_map.insert(std::pair<int,
				cliques::xyz_colour<float> >(i, cliques::xyz_colour<float>(hue,0.7,0.7)));
			++i;
		}
    	return colour_map;
	}
