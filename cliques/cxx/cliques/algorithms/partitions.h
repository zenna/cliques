...
void add_partition_to_set(c_partition *p, int num_nodes, int &num_partitions,
		boost::unordered_set<P, cliques::partition_hash, cliques::partition_equal> &all_partitions) {

	P new_partition;
	for (int i=0; i<num_nodes; ++i) {
		new_partition.add_element(i);
	}
...
void save_partition(c_partition *p, cliques::umap &partition_map, int &num_partitions)
{
	int offset = 0;
	int i;
							  ...
							  boost::unordered_set<P, cliques::partition_hash, cliques::partition_equal> &all_partitions)
{
	...
	if( neighbours != 0 )
	{
		...
		gen_partitions_prune_aux(comp_tmp, new_neighbours, forbidden, g, part,
								 nodes_to_place, part_union, partition_map,
	...
	else
	{
		...
		gen_partitions_prune_aux(new_comp_tmp, new_neighbours, new_forbidden, g, part,
								 nodes_to_place, part_union, partition_map,
								 num_partitions, all_partitions);
...
void find_connected_partitions(G &graph, boost::unordered_set<P, cliques::partition_hash, cliques::partition_equal> &all_partitions)
{
