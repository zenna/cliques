...
//
//    std::map<int, std::map<int, double> > all_basins;
//    int num_nodes = lemon::countNodes(transition_graph);
//    std::vector<int> node_to_basin(num_nodes, -1);
//
//    for (NodeIt node(transition_graph); node != lemon::INVALID; ++node) {
//        Node current_node = node;
//        std::vector<int> seen_nodes;
//        while (true) {
//            int node_id = transition_graph.id(current_node);
//
//            // Don't revisit paths
//            if (node_to_basin[node_id] >= 0) {
//                break;
//            }
//
//            int num_better_nodes = 0;
//            double best_energy = -10000;
//            for (OutArcIt arc(transition_graph, current_node); arc
//                    != lemon::INVALID; ++arc) {
//                num_better_nodes++;
//                Node neighbour = graph.runningNode(arc);
//                if (qualities[neighbour] > best_energy) {
//                    best_energy = qualities[neighbour];
...
//                cliques::output("maxima");
//                std::map<int, double> basin_to_probabilities;
//                for (auto itr = seen_nodes.begin(); itr != seen_nodes.end(); ++itr) {
//                    node_to_basin[*itr] = 1.0;
//                    basin_to_probabilities[*itr] = 1.0;
//                }
//
//                all_basins[node_id] = basin_to_probabilities;
//                break;
