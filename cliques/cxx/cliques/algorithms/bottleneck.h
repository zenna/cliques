...
*/
float find_bottleneck (lemon::SmartGraph &g, lemon::IterableValueMap<lemon::SmartGraph, lemon::SmartGraph::Edge, float> &weights,
		lemon::SmartGraph::Node &s, lemon::SmartGraph::Node &t, lemon::SmartGraph::EdgeMap<bool> &filter) {
	...
	if (num_edges == 1) {
		for (lemon::IterableValueMap<lemon::SmartGraph, lemon::SmartGraph::Edge, float>::ValueIt itr = weights.beginValue();itr != weights.endValue(); ++itr) {
			for (lemon::IterableValueMap<lemon::SmartGraph, lemon::SmartGraph::Edge, float>::ItemIt item_itr(weights, *itr); item_itr != lemon::INVALID; ++item_itr) {
	...
	if (comp[s] != comp[t]) {
		...
		for (cliques::setumap::iterator itr = edges_map.begin(); itr != edges_map.end(); ++itr) {
			std::vector<int> uv;
		...
		lemon::IterableValueMap<lemon::SmartGraph, lemon::SmartGraph::Edge, float> weights(temp_graph); // Map must be made after edges are added
		int i = 0;
		for (cliques::setumap::iterator itr = edges_map.begin(); itr != edges_map.end(); ++itr) {
