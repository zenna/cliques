...
template <typename G>
cliques::VectorPartition community_to_partition(G &graph, std::vector<int> comm, int outside_comm_set_id) {
...
bool will_removal_break_community(G &graph, std::vector<int> comm, NO &node_to_remove) {
...
std::vector<std::vector<int> > find_community_neighbours(G &graph,
    ...
    for (unsigned int i = 0; i < comm.size(); ++i) { // Better if I could just iterate thro
        ...
        if (forbidden.find(node_id) == forbidden.end()
            ...
            auto location = std::find(contracted_comm.begin(), contracted_comm.end(), node_id);
        ...
        for (typename G::IncEdgeIt e(graph, node); e != lemon::INVALID; ++e) {
            ...
            if (std::find(comm.begin(), comm.end(), neigh_node_id)
                    == comm.end() && forbidden.find(neigh_node_id) == forbidden.end()) {
...
bool is_community_maxima(G &graph, M &weights, std::vector<int> comm, QF compute_quality, double &stability) {
    cliques::VectorPartition comm_partition = community_to_partition(graph, comm, 0);
    auto internals = cliques::gen(compute_quality, graph, weights, comm_partition);
    ...
    for (auto itr = neighs.begin(); itr != neighs.end(); ++itr) {
        cliques::VectorPartition neigh_partition = community_to_partition(graph, *itr, 0);
        auto neigh_internals = cliques::gen(compute_quality, graph, weights, neigh_partition);
...
void find_optimal_community_huxley(G &graph, M &weights) {
