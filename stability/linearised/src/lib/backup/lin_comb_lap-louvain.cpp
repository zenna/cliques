...
#include "lin_comb_lap-louvain.h"

HGraph::HGraph(const Graph &gc):Graph(gc){
  nb_nodes_per_comm.resize(gc.degrees.size(),1); 
}
HGraph::HGraph():Graph(){};

CommunityFinder::CommunityFinder(const Graph &gc, const int nb_pass, const double min_modularity, const double timet)
  : c(gc,timet,gc.degrees.size()),nb_pass(nb_pass),min_modularity(min_modularity),time(timet)
{
}


Community::Community(const Graph &gc, double timet, int nb_nodes)
: size(gc.degrees.size()),g(gc), time(timet)
{
  n2c.resize(size);
  in.resize(size);

  // Antoine
  // Calculation of the mean degree and save of the initial number of nodes.
  // No need to calculate tot anymore.
  int temp=0;
...
Community::Community(const HGraph &gc, double timet, int nb_nodes)
: size(gc.degrees.size()),g(gc), time(timet)
{
  n2c.resize(size);
  in.resize(size);
...
Community::modularity() const {
  ...
  for (unsigned int i=0 ; i<size ; i++) {
	  ...
	  if(g.nb_nodes_per_comm[i]>0){
		  q += (time/m2)*(double)in[i]-(g.nb_nodes_per_comm[i]*g.nb_nodes_per_comm[i])*(1.0/nb_nodes_init)*(1.0/nb_nodes_init);
    }
  }
  return q;
}
...
Community::partition2graph_binary() const {
  ...
  const int comm_deg = comm_nodes.size();
  ...
  assert (g2.nb_nodes_per_comm.size()==0); 
  g2.nb_nodes_per_comm.resize(comm_deg,0);

  // Antoine
  // Transfer of the number of nodes per community to the new graph
  for(unsigned int n=0;n<g.degrees.size();n++){
...
Community::one_level(const double min_modularity, const int nb_pass) {
  ...
  //int nb_nodes_per_comm_temp[g.nb_nodes];
  std::vector<int> nb_nodes_per_comm_temp(g.degrees.size());
  nb_nodes_per_comm_temp = g.nb_nodes_per_comm;
...
//      if (node%1000000==0) {fprintf(stderr,"%d ",node); fflush(stderr);}
      ...
      remove(node, node_comm, ncomm.find(node_comm)->second, nb_nodes_per_comm_temp);
      ...
      for (std::map<int,int>::iterator it=ncomm.begin() ; it!=ncomm.end() ; it++) {
        double increase = modularity_gain(node, it->first, it->second, nb_nodes_per_comm_temp);
...
CommunityFinder::find_partition(const int display_level) const {
  ...
  const int numberinitial=c.numberofcommunities();
  
  double qual = c2.modularity();
  double new_qual = c2.one_level(min_modularity, nb_pass);

  if (display_level==-1)
    stdout_buffer << c.string_partition();

  HGraph g2 = c2.partition2graph_binary();
  int level=0;

  int numberofcommunities=c.numberofcommunities();
  while(new_qual-qual>min_modularity) {
    qual=new_qual;
    Community c2(g2, time, numberinitial);
